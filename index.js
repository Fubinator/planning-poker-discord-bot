...
client.on("message", (message) => {
  ...
  const fullCommand = args.shift(); // Command now holds the first word and args holds rest of them
  if (!fullCommand.startsWith(process.env.PREFIX)) return;
  const command = fullCommand.slice(1); // Removes the prefix
